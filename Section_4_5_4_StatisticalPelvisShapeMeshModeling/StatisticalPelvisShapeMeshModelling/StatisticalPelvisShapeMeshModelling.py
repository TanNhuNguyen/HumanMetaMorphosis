...
#************************************************************SUPPORTING BUFFERS*****************************************************************#
#***********************************************************************************************************************************************#
mainFolder = "../../../Data/Section_4_5_4_StatisticalPelvisShapeMeshModeling";
...
def trainTestSplit():
    ...
    fullSubjectIDs = readListOfStringsFromTextFile(os.path.join(mainFolder, "FemalePelvisIDs.txt"));
...
def buildPelvicMeshPCAModels():
    ...
    for subjectIndex in range(len(trainingSubjectIDs)):
        ...
        subjectMesh = o3d.io.read_triangle_mesh(os.path.join(mainFolder, "PelvisBoneMuscleMeshes", 
                                                             f"{trainingSubjectIDs[subjectIndex]}-PersonalizedPelvisBoneMuscleMesh.ply"));
    ...
    np.save(os.path.join(mainFolder, f"PelvisStructurePCAModels/Scaler_Comp_{numComps}.npy"), scaler);
    np.save(os.path.join(mainFolder, f"PelvisStructurePCAModels/PCAModel_Comp_{numComps}.npy"), pca);
    ...
    # Finished processing.
    print("Finished processing.");
def computeStatisticsForPelvicMeshPCAModels():
    # Initialize
    print("Initializing ...");

    # Reading the template pelvic mesh
    print("Reading the template pelvic mesh ...");
    templatePelvicMesh = o3d.io.read_triangle_mesh(os.path.join(mainFolder, "TemplatePelvicMesh.ply"));
    templateFaces = np.asarray(templatePelvicMesh.triangles);

    # Computing statistics for pelvic mesh PCA models with 1 number of components
    print("Computing statistics for pelvic mesh PCA models with 1 number of components ...");
    ## Load the scaler and the PCA model
    numComps = 1;
    scaler = np.load(os.path.join(mainFolder, f"PelvisStructurePCAModels/Scaler_Comp_{numComps}.npy"), allow_pickle=True).item();
    pca = np.load(os.path.join(mainFolder, f"PelvisStructurePCAModels/PCAModel_Comp_{numComps}.npy"), allow_pickle=True).item();
    ## Mean shape
    ...
    # Compute statistics for pelvic mesh PCA models with 100 components
    print("Computing statistics for pelvic mesh PCA models with 100 number of components ...");
    ## Load the scaler and the PCA model
    numComps = 100;
    scaler = np.load(os.path.join(mainFolder, f"PelvisStructurePCAModels/Scaler_Comp_{numComps}.npy"), allow_pickle=True).item();
    pca = np.load(os.path.join(mainFolder, f"PelvisStructurePCAModels/PCAModel_Comp_{numComps}.npy"), allow_pickle=True).item();
    ...
    # Finished processing.
    print("Finished processing.");
def computeTestingErrorsForPelvicMeshPCAModels():
    # Initializing
    print("Initializing ...");
    ...
    for subjectIndex in range(len(testingSubjectIDs)):
        ...
        ## Reading the subject's mesh
        subjectMesh = o3d.io.read_triangle_mesh(os.path.join(mainFolder, "PelvisBoneMuscleMeshes", 
                                                             f"{testingSubjectIDs[subjectIndex]}-PersonalizedPelvisBoneMuscleMesh.ply"));
        subjectVertices = np.asarray(subjectMesh.vertices);
        ## Flattening the vertices and adding to the data
    ...
    # Finished processing.
    print("Finished processing.");
def visualizePelvicMeshModel():
    # Initializing
    print("Initializing ...");
    def updatePelvicMeshModel(value, componentIndex, inScaler, inPCA, inTemplateFaces, inVisualizer):
        # Checking inputs
        if inScaler is None:
        ...
        inVisualizer.updateO3DMeshVertices("PelvicMesh", skullMeshVertices);
        inVisualizer.render();

    # Reading template pelvic mesh
    print("Reading template pelvic mesh ...");
    templatePelvicMesh = readO3DMesh(os.path.join(mainFolder, "TemplatePelvicMesh.ply"));
    templateFaces = np.asarray(templatePelvicMesh.triangles);
    ...
    visualizer.addO3DMesh("PelvicMesh", templatePelvicMesh, (0.8, 0.8, 0.8));
    ...
    visualizer.addVTKSliderBar("PC 1", pcaCoeffsMin, pcaCoeffsMax, 0.0, 
                               lambda value: updatePelvicMeshModel(value, 0, scaler, pca, templateFaces, visualizer));
