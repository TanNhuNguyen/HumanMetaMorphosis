...
def buildPCAModelsOfHeadAndSkullShapes():
    ...
    # Finished processing
    print("Finished processing.");
def trainSkullShapeToHeadShapeRelationship():
    # Initialize
    print("Training the relationship between skull and head shapes...");
    ...
    regressionModel.fit(skullPCAData, headPCAData);
    ## Save the trained regression model to file
    print("\t Save the trained regression model to file ...");
    np.save(os.path.join(mainFolder, "TrainedModels/TrainedSkullToHeadShapeRegressionModel.npy"), regressionModel);
    ...
    # Finished processing.
    print("Finished processing.");
def testSkullShapeToHeadShapeRelationship():
    # Initialize
    print("Testing the relationship between skull and head shapes...");

    # Read the template head shape and skull shape
    print("Read the template head shape and skull shape...");
    ...
    predictedHeadShapes = [];
    for i in range(len(skullTestData)):
        # Print progress
        visualizeProgressBar(i+1, len(skullTestData));
        # Get the skull shape vertex data
        skullShapeVertexData = skullTestData[i, :];
        # Scale the skull shape vertex data
        skullShapeVertexDataScaled = skullScaler.transform(skullShapeVertexData.reshape(1, -1));
        # Project the skull shape vertex data to the skull shape PCA space
        skullShapeVertexDataPCA = skullPCA.transform(skullShapeVertexDataScaled);
        # Predict the head shape using the regression model
        predictedHeadShapeVertexDataPCA = regressionModel.predict(skullShapeVertexDataPCA);
        # Reconstruct the head shape vertex data from the head shape PCA space
        predictedHeadShapeVertexDataScaled = headPCA.inverse_transform(predictedHeadShapeVertexDataPCA);
        # Unscale the predicted head shape vertex data
        predictedHeadShapeVertexData = headScaler.inverse_transform(predictedHeadShapeVertexDataScaled);
        # Reshape to (N, 3)
        predictedHeadShapeVertices = predictedHeadShapeVertexData.reshape(-1, 3);
        # Forming head shape mesh
        predictedHeadShape = o3d.geometry.TriangleMesh();
        predictedHeadShape.vertices = o3d.utility.Vector3dVector(predictedHeadShapeVertices);
        predictedHeadShape.triangles = o3d.utility.Vector3iVector(templateHeadShapeFaces);
        # Append to the list
        predictedHeadShapes.append(predictedHeadShape);
    
    # Compute the testing errors
    print("Compute the testing errors...");
    testingErrors = [];
    for i in range(len(predictedHeadShapes)):
        # Print progress
        visualizeProgressBar(i+1, len(predictedHeadShapes));
        # Get the predicted head shape
        predictedHeadShape = predictedHeadShapes[i];
        predictedHeadShapeVertices = np.asarray(predictedHeadShape.vertices);
        # Get the ground truth head shape
        groundTruthHeadShape = headTestData[i, :].reshape(-1, 3);
        # Compute the point-to-point distances
        distances = computePointsToPointsDistance(predictedHeadShapeVertices, groundTruthHeadShape);
    ...
    # Visualize the first predicted head shape vs ground truth
    print("Visualize the first predicted head shape vs ground truth...");
    ## Preparing data for visualization
    print("\t Preparing data for visualization ...");
    groundTruthHeadShape = o3d.geometry.TriangleMesh();
    groundTruthHeadShape.vertices = o3d.utility.Vector3dVector(headTestData[0, :].reshape(-1, 3));
    groundTruthHeadShape.triangles = o3d.utility.Vector3iVector(templateHeadShapeFaces);
    groundTruthHeadShape.compute_vertex_normals();
    predictedHeadShape = predictedHeadShapes[0];
    predictedHeadShape.compute_vertex_normals();
    ## Initialize visualizer
    print("\t Initialize visualizer ...");
    visualizer = Visualizer();
    visualizer.initializeRendering();
    ## Visualize the predicted head shape first
    print("\t Visualize the predicted head shape first ...");
    visualizer.addO3DMesh("predictedHeadShape", predictedHeadShape, (0.7, 0.7, 0.7));
    visualizer.render();
    ...
    ## Visualizer the ground truth head shape with the predicted head shape
    visualizer.addO3DMesh("predictedHeadShape", predictedHeadShape, (0.7, 0.7, 0.7));
    visualizer.addO3DMesh("groundTruthHeadShape", groundTruthHeadShape, (0.7, 0.0, 0.0));
    visualizer.render();
    visualizer.resetCamera();
    visualizer.startInteractionWindows();
...
def main():
    os.system("cls");
    testSkullShapeToHeadShapeRelationship();
